...
kotlinx.serialization tends to be an always-explicit framework when it comes to serialization strategies: normally,
...
However, in projects with big amount of files and classes in them it may be inconvenient to specify `@file:UseSerializers`
every time, especially for classes like `Date` or `Instant` that have fixed strategy of serialization across the project.
For such cases, it is possible to specify serializers using `typealias`es, as they preserve annotations, including serialization ones:
<!--- INCLUDE
import java.util.Date
import java.text.SimpleDateFormat
